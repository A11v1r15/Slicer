	...
	private IEnumerator MyEvent()
	{
		while(true)
		{
			...
			if(Mathf.Sin(Time.time) > Mathf.Sin(Cam.time)){
				LastMonster = Instantiate(Resources.Load("Prefabs/characters/Monster"+random.Next(1,5).ToString()) as GameObject, PlataformGenerator.EndPosition + new Vector3(0, 3f, 1f),Quaternion.identity) as GameObject;
			}
		}
	}
